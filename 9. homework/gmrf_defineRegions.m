function mrf = gmrf_defineRegions(params, mrf)
    ...
    for ind=1:params.NumOfClasses
        ...
        x1 =a.x1;
        x2 =a.x2;
        relevant_part_of_image_original = params.InputImage(y1:y2,x1:x2);
        relevant_part_of_image = double(relevant_part_of_image_original);
        Image_double = double(params.InputImage);
        mean_Image = mean(relevant_part_of_image(:));
        pixel_intensity_image = std(relevant_part_of_image(:));
        mrf.gauss_params{ind}.mean = mean_Image;
        mrf.gauss_params{ind}.dev = pixel_intensity_image;
        mrf.logProbs{ind} = -log(normpdf(Image_double, mrf.gauss_params{ind}.mean, mrf.gauss_params{ind}.dev));
